...
<div class="h-screen w-screen mx-auto p-4 relative">
  <Title>Double-click anywhere to create a new note</Title>
    ...
    <button id="btn-login" disabled onclick={() => console.log("login()")}>Log in</button>
    <button id="btn-logout" class="ml-2" disabled onclick={() => console.log("logout()")}>Log out</button>
  </div>
  <div id="card-layout">
    {#each cards as card}
